...
namespace Products.Api.Services
{
    ...
    internal sealed class FilterExpression : IFilterExpression
    {
        ...
        //(?<levelUp>\(*)?(?<left>([^\r\n\t\f\v \[]*)?)(?<params>\[.*\]\S*)?(\s* (?<operator>[operatorsList])\s* (?<right>(.)*))?
        // where [operatorsList] = bitAnd|eq|ne|co|sw|ew|ge|gt|isMemberOf|lt|matchesExpression|le|notBitAnd|notMatchesExpression
        ...
        private FilterExpression(string text, int group, int level)
        {
            ...
            MatchCollection matches = Expression.Value.Matches(this.Text);
            foreach (Match match in matches)
            {
                Group levelUpGroup = match.Groups[PatternGroupLevelUp];
                if (levelUpGroup.Success && levelUpGroup.Value.Any())
                {
                    Level += levelUpGroup.Value.Length;
                    Group += 1;
                }
                Group operatorGroup = match.Groups[PatternGroupOperator];
                if (operatorGroup.Success)
                {
                    Group leftGroup = match.Groups[PatternGroupLeft];
                    Group rightGroup = match.Groups[PatternGroupRight];
...
#pragma warning restore CA1303 // Do not pass literals as localized parameters
        ...
        [SuppressMessage("SonarLint", "S3776", Justification = "Avoid refactoring third-party code.")]
...
#pragma warning restore S125
        ...
        private static string Initialize<TOperator>()
        {
            ...
            foreach (TOperator op in comparisonOperatorValues)
            {
                if (buffer.Length > 0)
                {
...
#pragma warning restore CA1834
        ...
        private sealed class ComparisonValue : IComparisonValue
        {
